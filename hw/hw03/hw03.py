...
def pingpong(n):
    ...
    True
    """
    def inner(direction, k, incr):
    	if k==n:
    		return incr+direction
    	elif (has_seven(k)) or (k%7==0):
      		return inner((-1*direction), k+1, incr+direction)
    	else:
       		return inner(direction, k+1, incr+direction)
...
def riffle(deck):
    ...
    """
    return [deck[i//2] if i%2==0 else (i//2 + len(deck)//2) for i in deck]
...
def build_binary_ones(h):
    ...
    # hardcode them in this one but using for loop in build ones
